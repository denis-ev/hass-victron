...
class VictronSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def __init__(
        ...
        if description.slave not in (100, 225):
            self.entity_id = (
                f"{SWITCH_DOMAIN}.{DOMAIN}_{hub_slug}_{self.description.key}_{description.slave}".lower()
            )
        else:
            self.entity_id = f"{SWITCH_DOMAIN}.{DOMAIN}_{hub_slug}_{self.description.key}".lower()
