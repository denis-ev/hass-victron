...
class VictronBinarySensor(CoordinatorEntity, ButtonEntity):
    ...
    def __init__(
        ...
        if self.description.slave not in (100, 225):
            self.entity_id = f"{BUTTON_DOMAIN}.{DOMAIN}_{hub_slug}_{self.description.key}_{self.description.slave}".lower()
        else:
            self.entity_id = f"{BUTTON_DOMAIN}.{DOMAIN}_{hub_slug}_{self.description.key}".lower()
